---
title: "Carbon Flux in Andean Paramo Streams"
author: "Andrew"
date: "2019-11-27"
output:
  blogdown::html_page:
...
```{r filt14PpmPlot, echo=FALSE, message=FALSE, warning=FALSE, fig.width=10}
filt13 <- df%>%
...
plot24DO <- ggplot(filt34)+
  ...
  geom_vline(aes(xintercept = as.POSIXct("06:15", format = "%H:%M")))+
  geom_vline(aes(xintercept = as.POSIXct("18:21", format = "%H:%M")))+
  geom_point(aes(x = time, y = DO2_mg.L,group = day, col=tempC_436),lwd=2)+
  scale_color_gradient(name = "Discharge (L/s)", trans = "log",
                        breaks = my_breaks, labels = my_breaks)+
...
metabCO2%>%
  ...
  layout(
    ...
    xaxis = list(title = "Date"),
    yaxis = list(title = "CO<sub>2</sub> [&mu;mols m<sup>2</sup>d<sup>-1</sup>]")
  )
...
## <p>Daily Evasion & Metabolism in &#181;mols</p>
...
uplot <- up%>%
  ...
  layout(
    ...
    xaxis = list(title = "Date", titlefont = labelFont,
  showticklabels = TRUE,
  tickangle = 325,
  tickfont = xaxisFont,
  exponentformat = "E"),
    yaxis = list(title = "&mu;mols m<sup>2</sup>d<sup>-1</sup>", type = "log",titlefont = labelFont,
  showticklabels = TRUE,
  tickangle = 0,
  tickfont = yaxisFont,
  range = c(3,7)),
...
subplot(uplot,dplot, titleY = TRUE, titleX = TRUE)
