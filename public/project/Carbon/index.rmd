---
title: "Carbon Dioxide Evasion in Tropical Alpine Streams"
author: "Andrew"
date: "2019-11-27"
output:
  blogdown::html_page:
...
slides: 
summary: This is ongoing work in the Paramo region of Ecuador studying the amount of Carbon moving from high elevation streams to the atmosphere.
tags:
- Water
- Carbon
- Climate
...
[Download the poster from AGU '19 HERE](/files/AGU_Science_2019.pdf)
...
#change_here("C:/Users/HP/OneDrive - University of North Carolina at Chapel Hill/Ecuador/Github/Ecuador")
```
...
df <- read.csv(here::here("static/data/All_Stream_Data.csv"))%>%
...
pptDaily <- read.csv(here::here("static/data/ppt.csv"))
...
## Change in CO<sub>2</sub>

|       The first step in calculating evasion from the stream is to calculate change in $CO_{2}$ concentrations between two points within the stream. We will look at two pairs of sensors (1->3 & 3->4). Once we determine change in $CO_{2}$ concentration, we will convert ppm to $umol$ and then account for stream metabolism (respiration and photosynthesis) to calculate evasion. First, let's visualize change in $CO_{2}$ accross the pairs of sensors

 
```{r filt14PpmPlot, echo=FALSE, message=FALSE, warning=FALSE, fig.width=8}
...
So now that we know what the change in $CO_{2}$ between stations, it's time to account for stream metabolism. Stream metabolism includes Ecosystem Respiration (ER) and Gross Primary Production / phtosynthesis (GPP). Both of these processes include a 1:1 exchange of $CO_{2}$ and DO molecules. 
**Photosynthesis:** Use $CO_{2}$ and create O2 (1:1 ratio). 
...
```{r DOchange, echo=FALSE, message=FALSE, warning=FALSE, fig.width=8}
legendtitle <- list(yref='paper',xref="paper",y=.8,x=1.35, text="Discharge<br> [L/s<sup>-1</sup>]",showarrow=F)
...
## Convert CO<sub>2</sub> from concentration to mass
...
## Histograms of change in CO<sub>2</sub>
```{r CO2Hist, echo=FALSE, warning = FALSE, message=FALSE, fig.width=8}
...
pred1 <- read.csv(here("static/data/DO_1_Predictions.csv"))%>%
  select(date,GPP,ER)%>%
  mutate(Station = "1")
pred2 <- read.csv(here("static/data/DO_2_Predictions.csv"))%>%
  select(date,GPP,ER)%>%
  mutate(Station = "2")
pred4 <- read.csv(here("static/data/DO_4_Predictions.csv"))%>%
  select(date,GPP,ER)%>%
  mutate(Station = "4")
...
gpp <- metab%>%
  ...
  type = "date",
  tickformat = "%b %d"),
    yaxis = list(title = "GPP [g m<sup>-22</sup>d<sup>-1</sup>]", titlefont = labelFont,showticklabels = TRUE,
  tickangle = 0,
  tickfont = yaxisFont),
    legend = list (x = 1, y=.5),
    annotations = legendtitle
...
resp <- metab%>%
  ...
  type = "date",
  tickformat = "%b %d"),
    yaxis = list(title = "ER [g m<sup>-2</sup>d<sup>-1</sup>]", titlefont = labelFont,showticklabels = TRUE,
  tickangle = 0,
  tickfont = yaxisFont),
    legend = list (x = 1, y=.5),
    annotations = legendtitle
...
gpp
resp
#subplot(gpp,resp,shareY = TRUE)
```

## Estimate daily ER and GPP in umol of CO<sub>2</sub>
...
## Convert CO<sub>2</sub> Change into Daily Values
...
## As mol m<sup>-2</sup>d<sup>-1</sup>
```{r evasionMolsPerDayLog, echo=FALSE, warning=FALSE, message=FALSE, fig.width=8}
...
```{r eosPlotly, echo=FALSE, message=FALSE, warning = FALSE, fig.width=8}
...
## X/Y
```{r xyFlux, echo=FALSE, warning=FALSE, message = FALSE, fig.width=8}
