...
Okay, so just quickly before projections, take a look at the link on this slide to check out a bit more about the 'geoid' and to see how gravity actually changes all over the world.
---
### Projections
...
There are three factors in projecting a map onto a flat surface:

**1.** Shape   **2.** Distance   **3.** Direction

It is possible to maintain two of these when projecting, but not all three.
]
.center[
```{r echo=FALSE, out.height= 350, out.width= 700}
knitr::include_graphics("proj1.png")
```
]

<audio controls>
  <source src="/slides/intro_geographic_information/Day04/audio/d04s10.mp3" type="audio/mpeg">
  <p>Your browser doesn't support HTML5 audio.</p>
</audio>

???
The difference between a coordinate system and a projection system is the physical location on a globe (coordinate system) vs. the location on a flat surface (projection system). A coordinate system, for example will give you latitude and longitude based on a system of assigning those values to a physical location on the earth (example: from a GPS unit). As we know, the earth is not perfectly round so there have been different geographic coordinate systems over the years. In order to get a representation of the globe onto a map (flat surface) we must push and pull different parts, thus introducing some error. As you can see in the figure above, the coordinates of these three points are all the same in both projections and will always be the same. However, the distances between them change as the projection changes and we stretch and shrink different parts of the globe to make it lie flat.

---
.center[
```{r echo=FALSE, out.height= 450, out.width= 900}
knitr::include_graphics("proj2.png")
```
]

<audio controls>
  <source src="/slides/intro_geographic_information/Day04/audio/d04s11.mp3" type="audio/mpeg">
  <p>Your browser doesn't support HTML5 audio.</p>
</audio>

???
To give you an example, take these point locations above. We can project them different ways. I have first projected them into a Europe Equal Area Conic Projection and then reprojected them into UTM 11N. I then ran the ‘Generate Near Table’ tool on each of these differently projected point layers to calculate all of the distances between each of the points (using the planar method). The geographic coordinates of these points have not changed, but their location on a flat (planar) surface has shifted. Notice the different ‘NEAR_DIST’ values in the table above (units are meters). ArcGIS can sometimes make this hard to understand because it projects on the fly and is so good at adjusting distances between projections it can actually prevent you from making mistakes that would otherwise help you understand these concepts.

---
.center[
.med[
Changing the projection does not affect the coordinates of a place (Longitude and Latitude) on the surface of the earth, just how they are pushed and pulled to fit onto a flat surface.
]
```{r echo=FALSE, out.height= 350, out.width= 900}
knitr::include_graphics("proj3.png")
```
]

<audio controls>
  <source src="/slides/intro_geographic_information/Day04/audio/d04s12.mp3" type="audio/mpeg">
  <p>Your browser doesn't support HTML5 audio.</p>
</audio>

???

Here is one final example of identical points in the United States to illustrate how these places apparently have different distances between them when shown on a map, even though they are identical.
---
.mb[I strongly suggest you take a lot of time and explore these links to try and understand how projections really affect the way we view maps:
...
<audio controls>
  <source src="/slides/intro_geographic_information/Day04/audio/d04s13.mp3" type="audio/mpeg">
  <p>Your browser doesn't support HTML5 audio.</p>
</audio>

???
If you remember nothing else, remember this. Projecting a map is the process of stretching the globe onto a flat surface. When doing this, it is possible to preserve 2 out of the three variables of shape, distance and direction but not all three. A projection is not a GCS. A projection is the mathematical conversion from a GCS to a flat map.
