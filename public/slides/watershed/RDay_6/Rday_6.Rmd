...
---
```{r ptsMap, warning=FALSE, message=FALSE}
mapview(shp)%>%
  addMouseCoordinates()
```
...
We see from the table above that there are five different aquifers in this dataset. Let's focus on the one with the most sites, the [Floridan Aquifer](https://fl.water.usgs.gov/floridan/conceptual-model.html). We'll get the site numbers from the siteInfo we just created and create a new dataset. We'll also take this opportunity to trim the data, keeping only records after 1985
...
.pull-left[
.med[
Now we have a dataset with one aquifer, and we have corrected it so depth is now elevation in relation to sea level
]
